...
function CreationController (DatabaseConntection,ExpressApplication){
    ...
    this.app.use('/database/seed',async (req,res) => {
		await bcrypt.hash('drmario', 10,(err,hash) => {
			if(!err) {
				this.connection.query('INSERT INTO users(username,password,email,firstname,lastname,notif_token,apikey) VALUES("maxx730","' + hash + '","max.kinghorn@gmail.com","Max","Kinghorn","","' + crypto.createHash('sha1') + '"),("tolm640","' + hash + '","tolm.lamar@gmail.com","Tom","Lamar","",""),("ethan54","' + hash + '","ethan.clokels@gmail.com","Ethan","Clokey","","");',(err,result) => {
					if(!err) {
