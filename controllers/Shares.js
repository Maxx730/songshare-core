...
function SharesController(DatabaseConnection,ExpressApplication){
    ...
    this.app.get('/user/:id/shares',(req,res) => {
      ...
      let query = "select shared._id,shared.title,shared.artist,users.username,shared.art,shared.spotify_id from shared inner join users on shared.sharer=users._id inner join friends on friends.friend_id=users._id where (friends.user_id="+req.params.id+") or (shared.sharer="+req.params.id+") group by(shared._id)";
